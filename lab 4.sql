...
(select `order`.ord_amount, `order`.cus_id,customer.cus_gender, customer.cus_name from
...
*/
    ...
    product 
      ...
      on
      `order` .pricing _id = A1.pricing_id where `order`.cus_id=2;
...
`order`.PRICING_id=supplier_pricing.pricing.pricing_id and supplier_pricing.pro_id=product.pro_id;
/*
 ...
 */
 ...
 (select pro_id, mmin(supp_price) as Min_Price FROM supplier_pricing group  by pro_id)
 as t2 where t2.pro_id = product.pro_id)
 as t3 where t3.cat_id = category.cat_id group by t3.cat_id;
 /*
 7) Display the Id and name of the product ordered after "2021-10-05"
 */
 select product.pro_id,product.pro_name from `order` inner join supplier_pricing on supplier_pricing.pricing_id inner join product
 on product.pro_id=supplier_pricing.pro_id where `order` .ord_data>"2021-10-05";
 /*
